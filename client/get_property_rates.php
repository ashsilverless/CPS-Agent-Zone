...
$sql_del = "DELETE FROM `tbl_roompricetemp_so` WHERE uid = $user_id";
...
XML;
    ...
    if($total_records == '1'){
        ...
        foreach ($period as $dt) {
            ...
            $sql = "INSERT INTO `tbl_roompricetemp_so` (`pe_id`, `type_name`, `name`, `max`, `min`, `code`, `date`, `uid`) VALUES ('".$the_id."', '".rtrim($the_type_name)."', :rtitle, '".$the_max."', '".$the_min."', '".$the_code."','".$dt->format("Y-m-d")."','$user_id')";
		...
		foreach ($item as $data => $value){
            ...
            if ($pos === false && $pos2 === false) {
                ...
                foreach ($period as $dt) {
                    $sql = "INSERT INTO `tbl_roompricetemp_so` (`pe_id`, `type_name`, `name`, `max`, `min`, `code`, `date`, `uid`, `price_id_string`) VALUES ('".$value['id']."', '".rtrim($value['type_name']).' : '.$value['id']."', :rtitle, '".$value['max_occupancy']."', '".$value['min_occupancy']."', '".$value['code']."','".$dt->format("Y-m-d")."','$user_id',',')";
...
$rr_data = db_query("SELECT code FROM tbl_roompricetemp_so where uid = $user_id GROUP BY code;");
  $look2='';      
       ...
       $look2=$json;
            ...
            if (is_numeric($json['error'])) {
                ...
                foreach ($res_provisional as $data => $value){
                    $sql = "UPDATE `tbl_roompricetemp_so` SET `provisional` = '$value' WHERE `code` LIKE '".$room['code']."' AND date LIKE '".$data."' AND uid = $user_id; ";
                ...
                foreach ($res_allocation as $data => $value){
                    $sql = "UPDATE `tbl_roompricetemp_so` SET `allocation` = '$value' WHERE `code` LIKE '".$room['code']."' AND date LIKE '".$data."' AND uid = $user_id; ";
                ...
                foreach ($res_total as $data => $value){
                    $sql = "UPDATE `tbl_roompricetemp_so` SET `total` = '$value' WHERE `code` LIKE '".$room['code']."' AND date LIKE '".$data."' AND uid = $user_id; ";
            ...
            foreach($xmlobj->suppliers->supplier->services->service as $item) {
                ...
                foreach($item->prices as $prices):
                    ...
                    foreach($prices->price as $daily):
                        ...
                        if($rp == 'Special Offer'){
                            $sql = "UPDATE `tbl_roompricetemp_so` SET `basis` = '$basis_code', `so_price_adult` = '$pa', `so_price_child` = '$pc', `so_price_infant` = '$pi', `currency` = '$curr', `rate_type` = '$rp', `price_id_string` = CONCAT(`price_id_string`,'".$price_id.",') WHERE `pe_id` = '$pe_id' AND `date` LIKE '$sqldate' AND uid = $user_id;  ";
                        }else{
                            $sql = "UPDATE `tbl_roompricetemp_so` SET `basis` = '$basis_code', `price_adult` = '$pa', `price_child` = '$pc', `price_infant` = '$pi', `currency` = '$curr', `rate_type` = '$rp', `price_id_string` = CONCAT(`price_id_string`,'".$price_id.",') WHERE `pe_id` = '$pe_id' AND `date` LIKE '$sqldate' AND uid = $user_id;  ";
                        }
...
$pe_data = db_query("SELECT * FROM tbl_roompricetemp_so  WHERE uid = $user_id GROUP BY pe_id ;");
	...
	<div class="avail-property__body">
		...
		<?php foreach ($pe_data as $roomdate){  ?>
				...
				$conn->exec("SET CHARACTER SET $charset");      // Sets encoding UTF-8
				  ...
				  $sql = "SELECT * FROM `tbl_roompricetemp_so` WHERE `pe_id` = '".$roomdate['pe_id']."' AND (`date` >= '$start_date' AND `date` <= '$end_date') AND uid = $user_id ORDER BY date ASC";
		//debug($sql);
