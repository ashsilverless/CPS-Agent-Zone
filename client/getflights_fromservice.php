...
$fromMs = $_GET['from'];      $service_id = $_GET['service_id'];
...
$data = db_query("SELECT * FROM `tbl_flight_services`  where pe_id = '$service_id';");
...
if($data!=''){
    foreach ($data as $supplier):
        ...
        $depart_name = getField('tbl_airports','airport_name','id',$supplier['departure_id']);
        $arrival_name = getField('tbl_airports','airport_name','id',$supplier['arrival_id']);
...
foreach ($data as $supplier):
    ...
    $has_rates .= '<div class="row" style="background-color:white; margin:14px; padding:10px; border-radius:8px; border:1px solid #666;">
                           <div class="col-12">
                              <div class="row">
                                <div class="col-3"><p>Departing From</p><h2>'.$supplier['depart_from'].'</h2></div>
                                <div class="col-3"><p>Arriving at</p><h2>'.$supplier['arrive_at'].'</h2></div>
                                <div class="col-3"><p>Operator</p><h2>'.$supplier['operator'].'</h2></div>
                                <div class="col-3"><p>Flight Date</p><h2>'.date('D j M Y',strtotime($supplier['flight_date'])).'</h2></div>
                              </div>
                            </div>';
    ############################################
