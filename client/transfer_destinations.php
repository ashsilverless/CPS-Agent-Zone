...
foreach ($return_array as $return){
	...
	foreach ($array as $record){
		...
		$sql = "INSERT INTO `tbl_regions` (`id`, `country_id`, `region_name`, `region_desc`, `bl_live`, `created_by`, `created_date`) VALUES ('$pe_id', '$pe_parent_id', '$pe_title', '<no description>', '1', 'data transfer', '$str_date')";
...
$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
	...
	foreach ($array as $record){
		...
		$pe_id = $record['id'];     $pe_code = $record['code'];     $pe_title = sanSlash(record['name']);

		$sql = "INSERT INTO `tbl_regions` (`pe_id`, `pe_code`, `prop_title`, `created_by`, `created_date`) VALUES ('$pe_id', '$pe_code', '$pe_title', 'Data Transfer', '$str_date')";
