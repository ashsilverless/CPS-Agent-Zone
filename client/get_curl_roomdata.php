...
$rooms = getFields('tbl_rooms','prop_id',$prop_id,'=');     #   $tbl,$srch,$param,$condition
	...
	foreach ($rooms as $room){
		foreach ($period as $dt) {
			$sql = "INSERT INTO `tbl_pe_rr_rooms` (`prop_id`, `room_id`, `rr_id`, `pe_id`,  `room_date`, `created_by`, `created_date`) VALUES ('".$prop_id."', '".$room['id']."', '".$room['rr_id']."', '".$room['pe_id']."', '".$dt->format("Y-m-d")."','".$_SESSION['agent_name']."','$str_date')";
			$conn->exec($sql);
		}
	}
	$conn = null;
...
$conn = null;
	...
	$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		...
		$sql = "UPDATE `tbl_pe_data` SET `xml_str`=:resstring, `created_date` = '$str_date' WHERE `id` = '20' ;";
...
<div class="avail-property">
	...
	<div class="avail-property__body">
		...
		<?php $roomdates = getFields('tbl_rooms','prop_id',$prop_id,'=');     #   $tbl,$srch,$param,$condition
			foreach ($roomdates as $roomdate){  
				...
				$conn->exec("SET CHARACTER SET $charset");      // Sets encoding UTF-8
				  ...
				  $sql = "SELECT * FROM `tbl_pe_rr_rooms` WHERE `room_id` = '".$roomdate['id']."' AND (`room_date` >= '$start_date' AND `room_date` <= '$end_date') ORDER BY room_date ASC";
				  ...
				  while($row = $result->fetch(PDO::FETCH_ASSOC)) {
					...
					$maxdays < 50 ? $displayInfo = "<div class='avail-data ".$class."'><span>".$row['availability']."</span><span>".$row['currency'].$row['adult_sell_1']."</span></div>" : $displayInfo = "<div class='avail-data high ".$class."'><span>".$row['availability']."</span></div>";
					  
					  echo ("<div align='center' title='".$roomdate['room_title']."' data-toggle='roompopover' data-trigger='hover' data-html='true' data-content='Spaces available : ".$row['availability']."<br>Rate : ".$row['currency'].$row['adult_sell_1']."'>".$displayInfo."</div>");
