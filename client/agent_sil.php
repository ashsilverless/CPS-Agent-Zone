...
################################################################
        ...
        $startfrom = date("Y-m-d", strtotime("-6 months"));
...
$sql_del = "DELETE FROM `tbl_pe_agent_si` WHERE agent_id = $agent_id ; ";
...
foreach ($item as $data => $value){
	...
	$b->execute();
		...
		$query = 'SELECT SUM(amount) AS value_sum FROM `tbl_pe_agent_si` WHERE agent_id = "'.$agent_id.'" AND created_at >= "'.$startfrom.'";';
