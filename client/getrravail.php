		...
		foreach ($roomsdata as $roomdata){
            ...
            $total_records ++;
                ...
                foreach ($period as $dt) {
                    $sql = "INSERT INTO `tbl_roomavail` (`pe_id`, `type_name`, `name`, `max`, `min`, `code`, `date`, `uid`, `link_id`) VALUES ('".$roomdata['pe_room_id']."', '".$roomdata['room_type']."', :rtitle, '".$roomdata['max_occupancy']."', '".$roomdata['min_occupancy']."', '".$roomdata['rr_id']."','".$dt->format("Y-m-d")."','$user_id','$link_id')";
...
$pe_data = db_query("SELECT * FROM tbl_roomavail  WHERE uid = $user_id GROUP BY pe_id ;");
...
<div class="avail-property">
	...
	<div class="avail-property__body">
		...
		<?php foreach ($pe_data as $roomdate){  
                ...
                $availability = db_query("SELECT * FROM `tbl_roomavail` WHERE `pe_id` = '".$roomdate['pe_id']."' AND uid = $user_id ORDER BY date ASC");
