					...
					<?php $p_dates = getFields('tbl_itinerary_prop_dates','itinerary_id',$itin_id);  $prop_array = array();
					  foreach ($p_dates as $record): $prop_array[] = $record['prop_id'];?>
						<div class="daily-activity">
							...
							<a href="" data-propid="<?=$record['prop_id'];?>" data-propban="<?=getField('tbl_properties','prop_banner','id',$record['prop_id']);?>" data-propname="<?=getField('tbl_properties','prop_title','id',$record['prop_id']);?>" data-proplat="<?=trim(getField('tbl_properties','prop_lat','id',$record['prop_id']));?>" data-proplong="<?=trim(getField('tbl_properties','prop_long','id',$record['prop_id']));?>" class="button button__ghost button__icon-right flymetothemoon">Show on Map <i class="fas fa-eye"></i></a>
						</div>
					  <?php endforeach; ?>
				</div>
			</div>
		...
		<?php for($marker=0;$marker<count($plong);$marker++){?>
          {
            ...
            'properties': {
              'title': '<?=$pname[$marker]?>',
              'description': '<div style="height:85px;overflow:hidden;"><img src="../<?=$pbanner[$marker]?>" width="220"></div><a href="single-property.php?id=<?=$pid[$marker]?>" class="button"><i class="far fa-eye"></i>View Property</a>',
...
// add markers to map
  ...
  new mapboxgl.Marker(el)
    ...
    .setPopup(new mapboxgl.Popup({
      ...
      .setHTML('<p id="' + marker.properties.ident + '"><strong>' + marker.properties.title + '</strong></p>' + marker.properties.description + ''))
    .addTo(map);
});
	...
	$(".flymetothemoon").click(function(e){
		...
		new mapboxgl.Popup({offset: 25, className:'itin-popup' })
			.setLngLat([long,lat])
			.setHTML('<p><strong>' + propname + '</strong></p><div style="height:85px;overflow:hidden;"><img src="../'+propban+'" width="220"></div><a href="single-property.php?id='+propid+'" class="button"><i class="far fa-eye"></i>View Property</a>')
			.addTo(map);
		//###################################################//
     });
